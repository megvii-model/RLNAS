...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger, class_num):
...
def main(xargs):
    ...
    save_path = save_checkpoint({'epoch':0,
                                ...
                                model_base_path.format(0), logger)
    # start training
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        search_w_loss, search_w_top1, search_w_top5, search_a_loss, search_a_top1, search_a_top5 \
            = search_func(search_loader, network, criterion, w_scheduler, w_optimizer, a_optimizer, epoch_str,
                          xargs.print_freq, logger, class_num)
        search_time.update(time.time() - start_time)
        logger.log(
            '[{:}] search [base] : loss={:.2f}, accuracy@1={:.2f}%, accuracy@5={:.2f}%, time-cost={:.1f} s'.format(
                epoch_str, search_w_loss, search_w_top1, search_w_top5, search_time.sum))
        logger.log(
            '[{:}] search [arch] : loss={:.2f}, accuracy@1={:.2f}%, accuracy@5={:.2f}%'.format(epoch_str, search_a_loss,
                                                                                               search_a_top1,
                                                                                               search_a_top5))
        ...
        save_path = save_checkpoint({'epoch': epoch + 1,
                                     ...
                                     'a_optimizer': a_optimizer.state_dict(),
                                     'w_scheduler': w_scheduler.state_dict()},
                                    model_base_path.format(epoch+1), logger)
